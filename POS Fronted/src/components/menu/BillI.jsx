...
const Bill = () => {
  ...
  const orderMutation = useMutation({
    ...
    onSuccess: (resData) => {
      ...
      // ✅ 1️⃣ ORDER PLACED MESSAGE
      enqueueSnackbar("Order Placed Successfully!", {
        variant: "success",
      });

      // ✅ 2️⃣ TABLE BOOKED MESSAGE (thoda delay ke saath)
      setTimeout(() => {
      ...
      try {
        ...
        const options = {
          ...
          handler: async function (response) {
            ...
            // Create DB Order (TABLE BOOKED IN BACKEND)
            const orderData = {
              customerDetailes: {
                name: customerData.customerName,
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-5 mt-2">
        ...
        <h1 className="text-white font-bold">₹{total.toFixed(2)}</h1>
      </div>

      <div className="flex items-center justify-between px-5 mt-2">
        <p className="text-xs text-[#ababab]">Tax (5.25%)</p>
        <h1 className="text-white font-bold">₹{tax.toFixed(2)}</h1>
      </div>
      ...
      <div className="flex gap-3 px-5 mt-4">
        <button
          onClick={() => setPaymentMethod("Cash")}
          className={`w-full py-3 rounded ${paymentMethod === "Cash" ? "bg-gray-600" : "bg-gray-800"
            } text-white`}
        >
          Cash
        </button>
        ...
        <button
          onClick={() => setPaymentMethod("Online")}
          className={`w-full py-3 rounded ${paymentMethod === "Online" ? "bg-gray-600" : "bg-gray-800"
            } text-white`}
        >
          Online
        </button>
      ...
      {/* ACTIONS */}
      <div className="flex gap-3 px-5 mt-4">
        <button className="bg-blue-600 w-full py-3 rounded text-white">
          Print Receipt
        </button>
        <button
          onClick={handlePlaceOrder}
          className="bg-yellow-400 w-full py-3 rounded text-black font-semibold"
        >
          Place Order
        </button>
      </div>
